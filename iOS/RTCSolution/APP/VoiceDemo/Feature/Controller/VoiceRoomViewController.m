...
- (void)receivedRaiseHandInviteWithAudience {
    ...
    alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
        if ([action.title isEqualToString:@"确定"]) {
            [VoiceRTSManager confirmMicWithBlock:nil];
        }
    };
}
...
- (void)joinServerRoom:(void (^)(BOOL result))block {
    ...
    [VoiceRTSManager joinVoice:[LocalUserComponent userModel].name
...
- (void)loadDataWithMakeCoHost {
    ...
    __weak __typeof(self) wself = self;
    [[ToastComponent shareToastComponent] showLoading];
    [VoiceRTSManager raiseHandsMicWithBlock:^(RTSACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)loadDataWithLostCoHost {
    ...
    __weak __typeof(self) wself = self;
    [[ToastComponent shareToastComponent] showLoading];
    [VoiceRTSManager offSelfMicWithBlock:^(RTSACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)voiceReconnect {
    __weak __typeof(self) wself = self;
    [VoiceRTSManager reconnectWithBlock:^(VoiceControlRoomModel * _Nonnull roomModel, NSArray * _Nonnull users, RTSACKModel * _Nonnull ackModel) {
        NSString *type = @"";
        if (ackModel.result) {
            type = @"resume";
        } else if (ackModel.code == RTSStatusCodeUserIsInactive ||
                   ackModel.code == RTSStatusCodeRoomDisbanded ||
                   ackModel.code == RTSStatusCodeUserNotFound) {
            type = @"exit";
        } else {
...
- (void)hangUp {
    __weak __typeof(self) wself = self;
    [VoiceRTSManager leaveVoice:^(RTSACKModel * _Nonnull model) {
