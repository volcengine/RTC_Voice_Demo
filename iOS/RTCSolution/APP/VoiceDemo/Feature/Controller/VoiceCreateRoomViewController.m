...
- (void)joinButtonAction:(UIButton *)sender {
    ...
    [VoiceRTSManager createMeeting:self.roomNameTextFieldView.text
                          userName:self.userNameTextFieldView.text
                             block:^(NSString * _Nonnull token, VoiceControlRoomModel * _Nonnull roomModel, NSArray<VoiceControlUserModel *> * _Nonnull lists, RTSACKModel * _Nonnull model) {
        if (model.result) {
