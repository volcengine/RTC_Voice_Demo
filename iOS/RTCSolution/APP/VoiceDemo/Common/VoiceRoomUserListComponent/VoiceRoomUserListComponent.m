...
- (void)loadDataWithRaiseHandLists {
    ...
    [VoiceRTSManager getRaiseHandsWithBlock:^(NSArray<VoiceControlUserModel *> * _Nonnull userLists, RTSACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)loadDataWithAudienceLists {
    __weak __typeof(self) wself = self;
    [VoiceRTSManager getAudiencesWithBlock:^(NSArray<VoiceControlUserModel *> * _Nonnull userLists, RTSACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)updateTableViewWithModel:(VoiceControlUserModel *)userModel dataLists:(NSArray<VoiceControlUserModel *> *)dataLists {
    if (userModel.user_status == 0) {
        // 邀请上麦
        [VoiceRTSManager inviteMic:userModel.uid block:^(RTSACKModel * _Nonnull model) {
            if (!model.result) {
        ...
        // 举手 - 同意
        __weak __typeof(self)wself = self;
        [VoiceRTSManager agreeMic:userModel.uid block:^(RTSACKModel * _Nonnull model) {
            if (model.result) {
        ...
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            if ([action.title isEqualToString:@"确定"]) {
                [VoiceRTSManager offMic:userModel.uid block:^(RTSACKModel * _Nonnull model) {
                    if (model.result) {
