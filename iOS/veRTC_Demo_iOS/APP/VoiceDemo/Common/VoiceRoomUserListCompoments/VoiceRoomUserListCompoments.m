...
- (void)updateTableViewWithModel:(VoiceControlUserModel *)userModel dataLists:(NSArray<VoiceControlUserModel *> *)dataLists {
    if (userModel.user_status == 0) {
        // 邀请上麦
        [VoiceRTMManager inviteMic:userModel.user_id block:^(RTMACKModel * _Nonnull model) {
            if (!model.result) {
        ...
        // 举手 - 同意
        __weak __typeof(self)wself = self;
        [VoiceRTMManager agreeMic:userModel.user_id block:^(RTMACKModel * _Nonnull model) {
            if (model.result) {
        ...
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            if ([action.title isEqualToString:@"确定"]) {
                [VoiceRTMManager offMic:userModel.user_id block:^(RTMACKModel * _Nonnull model) {
                    if (model.result) {
