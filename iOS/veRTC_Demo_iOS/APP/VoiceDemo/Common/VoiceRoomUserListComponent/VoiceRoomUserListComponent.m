...
@interface VoiceRoomUserListComponent () <VoiceRoomTopSelectViewDelegate, VoiceRoomRaiseHandListsViewDelegate, VoiceRoomAudienceListsViewDelegate>
...
@implementation VoiceRoomUserListComponent
...
- (void)loadDataWithRaiseHandLists {
    ...
    [VoiceRTMManager getRaiseHandsWithBlock:^(NSArray<VoiceControlUserModel *> * _Nonnull userLists, RTMACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)loadDataWithAudienceLists {
    __weak __typeof(self) wself = self;
    [VoiceRTMManager getAudiencesWithBlock:^(NSArray<VoiceControlUserModel *> * _Nonnull userLists, RTMACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)updateTableViewWithModel:(VoiceControlUserModel *)userModel dataLists:(NSArray<VoiceControlUserModel *> *)dataLists {
    if (userModel.user_status == 0) {
        // 邀请上麦
        [VoiceRTMManager inviteMic:userModel.uid block:^(RTMACKModel * _Nonnull model) {
            if (!model.result) {
        ...
        // 举手 - 同意
        __weak __typeof(self)wself = self;
        [VoiceRTMManager agreeMic:userModel.uid block:^(RTMACKModel * _Nonnull model) {
            if (model.result) {
        ...
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            if ([action.title isEqualToString:@"确定"]) {
                [VoiceRTMManager offMic:userModel.uid block:^(RTMACKModel * _Nonnull model) {
                    if (model.result) {
