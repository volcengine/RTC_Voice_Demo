...
public class VoiceRTSClient extends RTSBaseClient {
    ...
    private static final String RECONNECT = "csReconnect";

    public VoiceRTSClient(@NonNull RTCVideo rtcVideo, @NonNull RTSInfo rtsInfo) {
        ...
        sendServerMessageOnNetwork(getRoomId(), params, VoiceResponse.class, new IRequestCallback<VoiceResponse>() {
            ...
            public void onSuccess(VoiceResponse data) {
                if (TextUtils.equals(cmd, INVITE_MIC)) {
                    SolutionToast.show(R.string.voice_invite_success);
                }
            }

            @Override
            public void onError(int errorCode, String message) {
                SolutionToast.show(R.string.voice_invite_failed);
            }
        });
    }
    ...
    public void switchMic(boolean isOn) {
        ...
        JsonObject params = getCommonParams(cmd);
        sendServerMessageOnNetwork(getRoomId(), params, VoiceResponse.class, null);
        VoiceRTCManager.ins().muteLocalAudioStream(!isOn);
    }
    ...
    public void requestReconnect(String roomId, IRequestCallback<VoiceReconnectResponse> callback) {
        JsonObject content = getCommonParams(RECONNECT);
        content.addProperty("room_id", roomId);
        sendServerMessageOnNetwork(roomId, content, VoiceReconnectResponse.class, callback);
    }
